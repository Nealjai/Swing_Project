...
Quality route (current code behavior):
- strict route: dynamic leadership quantile (`q90`) + actionability floor (`>= 0.58`), OR
- relaxed-by-playbook route with dynamic leadership thresholds:
  - `PULLBACK_ENTRY` / `TIGHT_BASE` / `LEADER_PULLBACK`: leadership `>= q85` and actionability `>= 0.55`
  - `BREAKOUT`: leadership `>= q88` and actionability `>= 0.58`
...
Backtesting tab active scenario dataset:
- `docs/data/backtest_active.json`

Note: legacy snapshots (`docs/data/backtest_active_10k.json`, `docs/data/backtest_active_30k.json`) may still exist in repo history/data, but current frontend loads the single active scenario file.
